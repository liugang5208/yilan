                ...
                <div class="card alert">
                    <div class="card-header">
                        ...
                        <div class="card-header-right-icon">
                            <ul>
                                <li class="doc-link">
                                    <a class="btn btn-xs btn-info" data-toggle="modal" data-target="#myModal">添加材料标签</a>
                                </li>
                            </ul>
                        </div>
                    </div>
        ...
        <div class="modal-dialog" role="document" style="width:40%;">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">添加材料标签</h4>
                </div>
    ...
    <div class="modal fade" id="myEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="width:40%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">更新材料标签</h4>
                </div>
                <div class="modal-body">
...
<script>
    ...
    app.controller('myCtrl', function ($scope, $http) {
        ...
        $scope.addor = function () {
            ...
            var baseurl = "<?php echo U('Core/addon','model=plate_cats_exc');?>";
            //
            $scope.commAjax(baseurl, data, function (res) {
                if (res.status != 1) {
        ...
        $scope.updateinfo = function (id) {
            $scope.commAjax("<?php echo U('Core/infos','model=plate_cats_exc');?>", {ids: id}, function (res) {
        ...
        $scope.updateCats = function () {
            ...
            param.ids = param.id;
            /////////
            $scope.commAjax("<?php echo U('Core/edits','model=plate_cats_exc');?>", param, function (res) {
                if (res.status !== 1) {
        ...
        $scope.dels = function (ids) {
            var param = {id: ids};
            $scope.commAjax("<?php echo U('Core/dels','model=plate_cats_exc');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                window.location.reload();
