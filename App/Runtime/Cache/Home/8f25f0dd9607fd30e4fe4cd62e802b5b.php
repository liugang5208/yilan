                                ...
                                <tr>
                                    <td class="edit2-td edit2-td-1">
                                        <div class="edit2-select-row">
                                            ...
                                            <select ng-model="infos.cat1" ng-change="onCatChange(1)" class="form-control edit2-select">
                                                <option value="">暂不选择</option>
                                                <option value="{{g.id}}" ng-repeat="g in allCateGroups">{{g.name}}</option>
                                            </select>
                                            <label class="edit2-label edit2-label-gap">材料:</label>
...
<script>
 ...
 function addor2() {
        ...
        if (!infos.isBase) {
            ...
            var pid3 = Number(infos.pid3 || 0);
            if (pid <= 0 && pid2 <= 0 && pid3 <= 0) {
                return swal('提示', '请至少选择一个材料', 'warning');
            }
        }
        ...
        data.ratio  = infos.ratio  || 0;
        data.ratio2 = infos.ratio2 || 0;
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (_selectedDirId) {
                    sessionStorage.setItem('autoSelectDirId', _selectedDirId);
                }
    ...
    app.controller('myCtrl', function ($scope, $http) {
        ...
        $scope.buildFilteredCateGroups = function(dirId) {
            ...
            (PAGE_CATE_LIST || []).forEach(function(g) {
                var t = Number(g.cate_type);
                if (t === 0) {
                    $scope.filteredCateGroups.push({ id: String(g.id), name: g.name, cate_type: t, children: g.children || [] });
                } else if (t === 2 && String(g.id) === String(dirId)) {
                    (g.children || []).forEach(function(cat) {
                        if (Number(cat.cate_type) === 1) {
                            $scope.filteredCateGroups.push({ id: String(cat.id), name: cat.name, cate_type: 1, children: cat.children || [] });
                        }
                    });
                }
            });
        ...
        $scope.onCatChange = function(idx) {
            ...
            var groupIsDerived = group && Number(group.cate_type) === 1;
        ...
        $scope.restoreCatSelections = function() {
            [[1,'pid'], [2,'pid2'], [3,'pid3']].forEach(function(pair) {
                ...
                var pid = $scope.infos[pidKey];
                if (pid && Number(pid) > 0) {
                    var found = PAGE_CATE_FLAT.find(function(item) { return String(item.id) === String(pid); });
                    if (found) {
                        $scope.infos['cat' + idx] = String(found.cate_group_id);
                        var group = ($scope.allCateGroups || []).find(function(g) { return String(g.id) === String(found.cate_group_id); });
                        var children = group ? (group.children || []) : [];
                        var groupIsDerived = group && Number(group.cate_type) === 1;
                        $scope.infos['subList' + idx] = children.filter(function(c) {
                            return groupIsDerived ? Number(c.pid) > 0 : Number(c.pid) === 0;
    ...
    $scope.onChangeRatio2 = function(){
        ...
        $scope.updateinfo2 = function (id) {
            $scope.commAjax("<?php echo U('NewLabel/addGet','model=new_label');?>", {cate_label_id: id}, function (res) {
                ...
                $scope.infos.ratio2 = 100;
                $scope.infos.ratio3 = 100;
                $scope.infos.end_ratio = 0;
                $scope.infos.cat1 = '';
                $scope.infos.cat2 = '';
                $scope.infos.cat3 = '';
                $scope.infos.subList1 = [];
                $scope.infos.subList2 = [];
                $scope.infos.subList3 = [];
                var cateGroup = (PAGE_CATE_LIST || []).find(function(g){ return String(g.id) === String(id); });
                $scope.infos.isBase = cateGroup ? cateGroup.cate_type == 0 : false;
                $scope.buildFullCateList();
                $scope.buildFilteredCateGroups(_selectedDirId);
                $("#myModal2").modal("show");
            });
        }
