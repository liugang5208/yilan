...
<div class="container-fluid">
                        ...
                        <div class="card-header-right-icon">
                            <ul>
                                <li class="doc-link" style="list-style: none;">
                                    <a class="btn btn-xs btn-info" data-toggle="modal" data-target="#myModal">添加银行卡号</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td><?php echo ($v['types']>0?'对公账户':'对私账户'); ?></td>
                                        <td><strong><?php echo ($v["bank"]); ?></strong></td>
                                        <td><?php echo ($v["bank_name"]); ?></td>
                                        <td><?php echo ($v["bank_tag"]); ?></td>
                                        <td><?php echo ($v["name"]); ?></td>
                                        <td><code><?php echo ($v["cardid"]); ?></code></td>
                                        <td><?php echo ($v["open_bank"]); ?></td>
                                        <td><?php echo ($v["open_bank_id"]); ?></td>
                                        <td>
                                            <span style="padding: 4px 10px; border-radius: 20px; font-size: 12px; <?php if(($v["status"]) == "1"): ?>background: #dcfce7; color: #166534;<?php else: ?>background: #fee2e2; color: #991b1b;<?php endif; ?>">
                                                <?php echo ($v['status']>0?'正常':'暂停使用'); ?>
                                            </span>
                                        </td>
                                        <td><?php echo (date('Y/m/d H:i:s',$v["times"])); ?></td>
                                        <td>
                                            <a class="btn btn-info btn-xs" ng-click="updateInfo('<?php echo ($v["id"]); ?>')">更新</a>
                                            <a class="btn btn-danger btn-xs" ng-click="dels('<?php echo ($v["id"]); ?>')" style="margin-left: 4px;">删除</a>
                                        </td>
                        ...
                        <div class="mobile-card-list">
                            <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="m-card-item">
                                    <div class="m-card-header">
                                        <span class="m-card-title"><?php echo ($v["bank"]); ?> (<?php echo ($v["bank_name"]); ?>)</span>
                                        <span class="m-card-id">ID: <?php echo ($v["id"]); ?></span>
                                    </div>
                                    <div class="m-card-body">
                                        <div class="m-card-row">
                                            <span class="m-card-label">卡片类型</span>
                                            <span class="m-card-value"><?php echo ($v['types']>0?'对公账户':'对私账户'); ?></span>
                                        </div>
                                        <div class="m-card-row">
                                            <span class="m-card-label">银行标记</span>
                                            <span class="m-card-value"><?php echo ($v["bank_tag"]); ?></span>
                                        </div>
                                        <div class="m-card-row">
                                            <span class="m-card-label">开户名称</span>
                                            <span class="m-card-value"><?php echo ($v["name"]); ?></span>
                                        </div>
                                        <div class="m-card-row">
                                            <span class="m-card-label">银行卡号</span>
                                            <span class="m-card-value" style="color: var(--apple-primary);"><?php echo ($v["cardid"]); ?></span>
                                        </div>
                                        <div class="m-card-row">
                                            <span class="m-card-label">开户行</span>
                                            <span class="m-card-value"><?php echo ($v["open_bank"]); ?></span>
                                        </div>
                                        <div class="m-card-row">
                                            <span class="m-card-label">开户行号</span>
                                            <span class="m-card-value"><?php echo ($v["open_bank_id"]); ?></span>
                                        </div>
                                        <div class="m-card-row">
                                            <span class="m-card-label">状态</span>
                                            <span class="m-card-value"><?php echo ($v['status']>0?'正常':'暂停使用'); ?></span>
                                        </div>
                                        <div class="m-card-row">
                                            <span class="m-card-label">记录时间</span>
                                            <span class="m-card-value"><?php echo (date('Y/m/d H:i:s',$v["times"])); ?></span>
                                        </div>
                                    </div>
                                    <div class="m-card-footer">
                                        <a class="btn btn-info" ng-click="updateInfo('<?php echo ($v["id"]); ?>')">更新</a>
                                        <a class="btn btn-danger" ng-click="dels('<?php echo ($v["id"]); ?>')">删除</a>
                                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel">添加银行卡号</h4>
                    ...
                    <form name="addon">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    <label>银行名称</label>
                                    <input type="text" name="bank" class="form-control" placeholder="请输入银行简称"/>
                                </div>
                                <div class="form-group">
                                    <label>银行全称</label>
                                    <input type="text" name="bank_name" class="form-control" placeholder="请输入银行全称"/>
                                </div>
                                <div class="form-group">
                                    <label>银行标记</label>
                                    <input type="text" name="bank_tag" class="form-control" placeholder="请输入银行标记"/>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>开户名</label>
                                    <input type="text" name="name" class="form-control" placeholder="请输入开户名"/>
                                </div>
                                <div class="form-group">
                                    <label>银行卡号</label>
                                    <input type="text" name="cardid" class="form-control" placeholder="请输入银行卡号"/>
                                </div>
                                <div class="form-group">
                                    <label>开户行</label>
                                    <input type="text" name="open_bank" class="form-control" placeholder="请输入开户行"/>
                                </div>
                                <div class="form-group">
                                    <label>开户行号</label>
                                    <input type="text" name="open_bank_id" class="form-control" placeholder="请输入开户行号"/>
                                </div>
                            </div>
                        </div>
                    </form>
    ...
    <div class="modal fade" id="myEdits" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel">修改银行卡号</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>卡片类型</label>
                                <select name="types" class="form-control" ng-model="info.types">
                                    <option value="0">对私账户</option>
                                    <option value="1">对公账户</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>银行账户类别</label>
                                <select name="check" class="form-control" ng-model="info.check">
                                    <option value="0">不含发票</option>
                                    <option value="1">普通发票</option>
                                    <option value="2">专用发票</option>
                                </select>
                            ...
                            <div class="form-group">
                                <label>银行名称</label>
                                <input type="text" name="bank" class="form-control" ng-model="info.bank"/>
                            </div>
                            <div class="form-group">
                                <label>银行全称</label>
                                <input type="text" name="bank_name" class="form-control" ng-model="info.bank_name"/>
                            </div>
                            <div class="form-group">
                                <label>银行标记</label>
                                <input type="text" name="bank_tag" class="form-control" ng-model="info.bank_tag"/>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>开户名</label>
                                <input type="text" name="name" class="form-control" ng-model="info.name"/>
                            </div>
                            <div class="form-group">
                                <label>银行卡号</label>
                                <input type="text" name="cardid" class="form-control" ng-model="info.cardid"/>
                            </div>
                            <div class="form-group">
                                <label>开户行</label>
                                <input type="text" name="open_bank" class="form-control" ng-model="info.open_bank"/>
                            </div>
                            <div class="form-group">
                                <label>开户行号</label>
                                <input type="text" name="open_bank_id" class="form-control" ng-model="info.open_bank_id"/>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function addor() {
        ...
        var baseurl = "<?php echo U('Core/addon','model=sys_banks');?>";
        //////
        $.ajax({
            url: baseurl,
            type: "post",
    ...
    app.controller('myCtrl', function ($scope, $http) {
        ...
        $scope.updateInfo = function (id) {
            $scope.commAjax("<?php echo U('Core/infos','model=sys_banks');?>", {ids: id}, function (res) {
                $scope.info = res.data;
                $("#myEdits").modal("show");
            });
        }
        ...
        $scope.updateOp = function () {
            ...
            /////////
            $.ajax({
                url: "<?php echo U('Core/edits','model=sys_banks');?>",
                type: "post",
                dataType: 'json',
                data: param,
        ...
        $scope.dels = function (ids) {
            var param = {id: ids};
            $scope.commAjax("<?php echo U('Core/dels','model=sys_banks');?>", param, function (res) {
                if (res.status !== 1) {
