                ...
                <div class="card alert">
                    <div class="card-header">
                        ...
                        <div class="card-header-right-icon">
                            <ul>
                                <li class="doc-link">
                                    <a class="btn btn-xs btn-info" data-toggle="modal" data-target="#myModal">添加物流城市</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <?php if(is_array($list["list"])): $k = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><tr>
                                        ...
                                        <td>
                                            <a href="<?php echo U('Trans/index','id='.$v['id']);?>" class="btn btn-info btn-xs">物流列表</a>
        ...
        <div class="modal-dialog" role="document" style="width:35%;">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">添加物流城市</h4>
                </div>
    ...
    <div class="modal fade" id="myEdits" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="width:35%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">修改物流城市</h4>
                </div>
                <div class="modal-body">
...
<script>
    ...
    app.controller('myCtrl', function ($scope, $http) {
        ...
        $scope.addor = function () {
            ...
            /////////
            $.ajaxFileUpload({
                url: "<?php echo U('Core/addon','model=trans_city');?>",
                secureuri: false,
                fileElementId: "file",
                dataType: 'json',
                data: data,
        ...
        $scope.updateInfo = function (id) {
            $scope.commAjax("<?php echo U('Core/infos','model=trans_city');?>", {ids: id}, function (res) {
        ...
        $scope.updateOp = function () {
            ...
            param.ids = param.id;
            /////
            $scope.commAjax("<?php echo U('Core/edits','model=trans_city');?>", param, function (res) {
                if (res.status !== 1) {
        ...
        $scope.dels = function (ids) {
            var param = {id: ids};
            $scope.commAjax("<?php echo U('Core/dels','model=trans_city');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                window.location.reload();
