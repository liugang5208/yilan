            ...
            <div class="card alert">
                <div class="card-header">
                    ...
                    <div class="card-header-right-icon">
                        <ul>
                            <li class="doc-link">
                                <a class="btn btn-xs btn-info" data-toggle="modal" data-target="#myModal">添加商品模板</a>
                            </li>
                        </ul>
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <?php if(is_array($list["list"])): $k = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><tr>
                                    ...
                                    <td>
                                        <a href="<?php echo U('Goods/tmps_log','id='.$v['id']);?>" class="btn btn-info btn-xs">查看</a>
                                        <a ng-click="reModel('<?php echo ($v["id"]); ?>')" class="btn btn-info btn-xs">重新上传</a>
                                        <a ng-click="updateInfo('<?php echo ($v["id"]); ?>')" class="btn btn-info btn-xs">编辑</a>
                                        <a ng-click="clear('<?php echo ($v["id"]); ?>')" class="btn btn-danger btn-xs">清空</a>
        ...
        <div class="modal-dialog" role="document" style="width:35%;">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">添加商品模板</h4>
                </div>
                    ...
                    <form name="up">
                        ...
                        <div class="form-group">
                            ...
                            <select name="types" class="form-control">
                                <option value="2">单商品模板</option>
                                <option value="1">多商品模板</option>
                            </select>
                        </div>
                        <div class="help-block">
                            <a href="/Public/file/sig.xlsx" class="btn btn-link" download="单商品模板">单商品模板下载</a>
                            <a href="/Public/file/mor.xlsx" class="btn btn-link" download="多商品模板">多商品模板</a>
                        </div>
                    </form>
    ...
    <div class="modal fade" id="myEdits" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="width:35%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">更新商品模板信息</h4>
                </div>
                <div class="modal-body">
    ...
    <div class="modal fade" id="myChange" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="width:35%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">重新上传商品模板信息</h4>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-primary" ng-click="readdor()">更新</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    app.controller('myCtrl', function ($scope, $http) {
        ...
        $scope.addor = function () {
            var temp = $("form[name='up']").serializeArray();
            var data = objToArray(temp);
            data.type = 1;
            /////////
            $.ajaxFileUpload({
                url: "<?php echo U('Goods/tmps_addon');?>",
                secureuri: false,
                fileElementId: "file",
                dataType: 'json',
                data: data,
         ...
         * @returns {undefined}
         */
        $scope.readdor = function () {
            var temp = $("form[name='reupload']").serializeArray();
            var data = objToArray(temp);
            data.ids = $scope.liid;
            data.type = 2;
            /////////
        ...
        $scope.updateInfo = function (id) {
            $scope.commAjax("<?php echo U('Core/infos','model=sys_tmps');?>", {ids: id}, function (res) {
        ...
        $scope.updateOp = function () {
            ...
            /////////
            $.ajaxFileUpload({
                url: "<?php echo U('Core/edits','model=sys_tmps');?>",
                secureuri: false,
        ...
        $scope.clear = function (ids) {
            var param = {id: ids};
            $scope.commAjax("<?php echo U('Goods/tmps_clear');?>", param, function (res) {
                if (res.status !== 1) {
        ...
        $scope.dels = function (ids) {
            var param = {id: ids};
            $scope.commAjax("<?php echo U('Goods/tmps_dels');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                window.location.reload();
