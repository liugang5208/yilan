<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html lang="en">

    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>易缆管理中心</title>
        <!-- ================= Favicon ================== -->
        <!-- Styles -->
        <link href="/Public/assets/css/lib/font-awesome.min.css" rel="stylesheet">
        <link href="/Public/assets/css/lib/themify-icons.css" rel="stylesheet">
        <link href="/Public/assets/css/lib/menubar/sidebar.css" rel="stylesheet">
        <link href="/Public/assets/css/lib/bootstrap.min.css" rel="stylesheet">
        <link href="/Public/assets/css/lib/unix.css" rel="stylesheet">
        <link href="/Public/assets/css/style.css" rel="stylesheet">
        <!-- jquery vendor -->
        <script src="/Public/assets/js/lib/jquery.min.js"></script>
        <script src="/Public/assets/js/lib/jquery.nanoscroller.min.js"></script>
        <script src="/Public/assets/js/lib/bootstrap.min.js"></script>
        <!-- Angularjs -->
        <script src="/Public/libs/angular/angular.min.js"></script>

        <!-- sweetalert -->
        <link href="/Public/libs/sweet-alert2/sweetalert2.min.css" rel="stylesheet">
        <script src="/Public/libs/sweet-alert2/sweetalert2.min.js"></script>

        <!----->
        <link href="/Public/libs/swiper/css/swiper.min.css" rel="stylesheet">
        <script src="/Public/libs/swiper/js/swiper.min.js"></script>

        <script src="/Public/libs/layer/layer.js"></script>
        <script src="/Public/libs/common.js"></script>
        <script src="/Public/libs/ajaxfileupload.js?v=3"></script>
        <style>
            .table{
                font-size:12px;
            }
            .none{
                display:none!important;
            }
            .sidebar .nano-content > ul li > a{
                padding: 8px 20px;
            }
        </style>
    </head>

    <body>

        <div class="sidebar sidebar-hide-to-small sidebar-shrink sidebar-gestures">
            <div class="nano">
                <div class="nano-content">
                    <ul>
                        <li class="label">管理功能</li>
                        <li><a href="<?php echo U('Bords/index');?>"><i class="ti-home"></i> 概览</a></li>
                        <li class="active open">
                            <a class="sidebar-sub-toggle">
                                <i class="ti-harddrives"></i> 用户管理 <span class="sidebar-collapse-icon ti-angle-down"></span>
                            </a>
                            <ul>
                                <li><a href="<?php echo U('Users/index');?>">注册用户</a></li>
                                <li><a href="<?php echo U('Users/logrs');?>">登录统计</a></li>
                                <li><a href="<?php echo U('Users/search');?>">搜索记录</a></li>
                                <!--li><a href="<?php echo U('Users/label');?>">禁止区域</a></li-->
                                <li><a href="<?php echo U('Users/levels');?>">用户等级</a></li>
                                <!--li><a href="<?php echo U('Users/cnlogs');?>">账户操作记录</a></li-->
                                <li><a href="<?php echo U('Users/mess');?>">消息管理</a></li>
                            </ul>
                        </li>
                        <li><a href="<?php echo U('Task/index');?>"><i class="ti-harddrives"></i> 材料价格任务执行更新</a></li>
                        <li><a href="<?php echo U('NewLabel/index');?>"><i class="ti-harddrives"></i> 材料价格分类</a></li>
                        <li><a href="<?php echo U('NewCate/index');?>"><i class="ti-harddrives"></i> 公式计算</a></li>
                        <li><a href="<?php echo U('NewTax/index');?>"><i class="ti-harddrives"></i> 税率标签</a></li>
                        <!--<li><a href="<?php echo U('Goods/cats_exc');?>"><i class="ti-harddrives"></i> 材料标签</a></li>-->
                        <li><a href="<?php echo U('Goods/cats');?>"><i class="ti-harddrives"></i> 商品分类</a></li>
                        <!--li><a href="<?php echo U('Goods/tmps');?>"><i class="ti-harddrives"></i> 商品模板</a></li-->
                        <li><a href="<?php echo U('Goods/liner');?>"><i class="ti-harddrives"></i> 线缆板块</a></li>
                        <li><a href="<?php echo U('Goods/peita');?>"><i class="ti-harddrives"></i> 配套板块</a></li>
                        <li><a href="<?php echo U('Shops/index');?>"><i class="ti-harddrives"></i> 店铺管理</a></li>
                        <li><a href="<?php echo U('Order/index');?>"><i class="ti-harddrives"></i> 订单管理</a></li>
                        <!--li><a href="<?php echo U('Shops/buyer');?>"><i class="ti-harddrives"></i> 采购管理</a></li>
                        <li><a href="<?php echo U('Shops/recyle');?>"><i class="ti-harddrives"></i> 回收管理</a></li-->
                        <!--<li><a href="<?php echo U('Trans/citys');?>"><i class="ti-harddrives"></i> 物流板块</a></li>-->
                        <!--li>
                            <a class="sidebar-sub-toggle">
                                <i class="ti-harddrives"></i> 知识板块 <span class="sidebar-collapse-icon ti-angle-down"></span>
                            </a>
                            <ul>
                                <li><a href="<?php echo U('Know/cats');?>">知识分类</a></li>
                                <li><a href="<?php echo U('Know/index');?>">知识文章</a></li>
                            </ul>
                        </li-->
                        <li><a href="<?php echo U('Know/advs');?>"><i class="ti-harddrives"></i> 广告管理</a></li>
                        <li><a href="<?php echo U('Know/helps');?>"><i class="ti-harddrives"></i> 帮助中心</a></li>
                        <li><a href="<?php echo U('Know/sysc');?>"><i class="ti-harddrives"></i> 系统文本</a></li>
                        <li><a href="<?php echo U('Sysc/banks');?>"><i class="ti-harddrives"></i> 银行卡号</a></li>
                        <li><a href="<?php echo U('Version/index');?>"><i class="ti-harddrives"></i> 系统版本</a></li>
                        <li><a href="<?php echo U('Bar/index');?>"><i class="ti-harddrives"></i> tab栏控制</a></li>
                         <li><a href="<?php echo U('ReportInfo/index');?>"><i class="ti-harddrives"></i>报价单管理</a></li>
                         <!--<li><a href="<?php echo U('Config/customer_config');?>"><i class="ti-harddrives"></i>客服配置</a></li>-->

                        <li><a href="<?php echo U('Index/index');?>"><i class="ti-close"></i> 退出登录</a></li>
                    </ul>
                </div>
            </div>
        </div>
        <!-- /# sidebar -->

        <div class="header">
            <div class="pull-left">
                <div class="logo"><a href="<?php echo U('Bords/index');?>"><span>易缆管理中心</span></a></div>
            </div>

            <div class="pull-right p-r-15">
                <ul>

                    <li class="header-icon dib">
                        <img class="avatar-img" src="/Public/assets/images/avatar/1.jpg" alt="" />
                        <span class="user-avatar">管理员 <i class="ti-angle-down f-s-10"></i></span>

                        <div class="drop-down dropdown-profile">
                            <div class="dropdown-content-body">
                                <ul>
                                    <li><a href="javascript:;"><i class="ti-user"></i> <span>密码管理</span></a></li>
                                    <li><a href="<?php echo U('Index/index');?>"><i class="ti-power-off"></i> <span>退出登录</span></a></li>
                                </ul>
                            </div>
                        </div>
                    </li>
                </ul>
            </div>
        </div>




        <div class="content-wrap">
            <div class="main">

                <style>
    .form-horizontal .control-label{ padding-top:5px;}
    .form-group{ margin-bottom:2px;}
    .border-dark{ border:1px solid #e7e7e7; padding:5px 0px;}
    label{ vertical-align:middle; margin-bottom:0px;}

    .form-block{ cursor:pointer; height:64px; line-height:64px;}
    .nav-tabs li{ cursor:pointer;}

    .table>thead>tr>th{ padding:4px; text-align:center;}
    .table>thead>tr>td{ padding:4px; text-align:center;}
    .table>tbody>tr>td{ padding:4px; text-align:center;}
    .table>thead>tr>td .form-control{ text-align:center;}
    .table>tbody>tr>td .form-control{ text-align:center;}

    .form-width label{ display:inline-block; width:24%!important;}
    .form-width input{ display:inline-block; width:70%!important;}

    .myTempBody{ height:500px; overflow-y:scroll;}
</style>

<div class="container-fluid" ng-app="myApp" ng-controller="myCtrl">

    <div class="row">
        <div class="col-lg-8 p-r-0 title-margin-right">
            <div class="page-header">
                <div class="page-title">
                    <h1>属性分类商品管理</h1>
                </div>
            </div>
        </div>
        <!-- /# column -->
        <div class="col-lg-4 p-l-0 title-margin-left">
            <div class="page-header">
                <div class="page-title">
                    <ol class="breadcrumb text-right">
                        <li><a href="<?php echo U('Goods/plate_conts','id='.$info['pid']);?>">细分商品</a></li>
                        <li class="active">属性分类商品管理</li>
                    </ol>
                </div>
            </div>
        </div>
        <!-- /# column -->
    </div>
    <!-- /# row -->
    <section id="main-content">
        <div class="row">
            <div class="col-lg-12">
                <div class="card alert">
                    <!--<div class="card-header">-->
                    <!--    <h4>属性分类商品管理</h4>-->
                    <!--</div>-->
                    <div class="card-body p-t-10">

                        <div style="display:flex;justify-content:space-between">
                            <div>商品价格组成</div>
                            
                            <div>
                                 <a class="btn btn-xs btn-danger btn-info" ng-click="delAllPrice()" data-toggle="modal" >一键删除</a>
                                 <a class="btn btn-xs btn-info" ng-click="copyPrice()" data-toggle="modal" >复制上一版块的内容</a>
                                 <a class="btn btn-xs btn-info" ng-click="priceAdd()" data-toggle="modal" >添加</a>
                            </div>
                       
                        </div>
                        
                        <div class="row border-dark">
                            <div class="col-md-11">
                                <div class="row">
                                    <!--<div class="col-md-2">-->

                                    <!--    <form class="form-horizontal">-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">上调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.up_a" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">下调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.down_a" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--    </form>-->

                                    <!--</div>-->
                                    <!--<div class="col-md-2">-->

                                    <!--    <form class="form-horizontal">-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">上调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.up_b" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">下调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.down_b" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--    </form>-->

                                    <!--</div>-->
                                    <!--<div class="col-md-2">-->

                                    <!--    <form class="form-horizontal">-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">上调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.up_c" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">下调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.down_c" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--    </form>-->

                                    <!--</div>-->
                                    <!--<div class="col-md-2">-->

                                    <!--    <form class="form-horizontal">-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">上调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.up_d" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">下调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.down_d" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--    </form>-->

                                    <!--</div>-->
                                    <!--<div class="col-md-2">-->

                                    <!--    <form class="form-horizontal">-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">上调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.up_e" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">下调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.down_e" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--    </form>-->

                                    <!--</div>-->
                                    <!--<div class="col-md-2">-->

                                    <!--    <form class="form-horizontal">-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">上调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.up_f" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--        <div class="form-group">-->
                                    <!--            <label class="col-sm-4 control-label">下调</label>-->
                                    <!--            <div class="col-sm-8">-->
                                    <!--                <input type="text" class="form-control input-sm" ng-model="blank.down_f" value="0">-->
                                    <!--            </div>-->
                                    <!--        </div>-->
                                    <!--    </form>-->

                                    <!--</div>-->
                            <!--价格组成-->
                             <div style="overflow-x:auto;white-space: nowrap;display:flex;align-content: space-around;width:100%;flex-direction: row;">
                                
                                <div style="display:flex;justify-content:space-between;width:150px;margin-left:5px;border: 0.5px solid black;" value="{{v.id}}" ng-repeat="v in price_list">
                                    <div style="display:flex;flex-direction:column" ng-click="priceEdit(v.id)">
                                        <div>
                                              {{v.name}}
                                        </div>
                                       <div>
                                              {{v.ratio}}
                                        </div>
                                    </div>
                                    
                                    <div style="">
                                         <!--<a ng-click="priceEdit(v.id)" style="font-size: 12px;" class="glyphicon glyphicon-pencil"></a>-->
                                            <a class="btn btn-danger btn-xs" ng-click="delsPrice(v.id)">-</a>
                                            
                                        
                                    </div>
                                
                                </div>
                           
                            </div>        
                                    
                                    
                            <!--    <table class="table">-->
                            <!--    <thead>-->
                            <!--        <tr>-->
                                        <!--<th>#ID</th>-->
                            <!--            <th>名称</th>-->
                            <!--            <th>百分比</th>-->
                            <!--            <th width="20%">操作</th>-->
                            <!--        </tr>-->
                            <!--    </thead>-->
                            <!--    <tbody>-->

                            <!--     <tr ng-repeat="x in price_list">-->
                                        <!--<th scope="row"><?php echo ($v["id"]); ?></th>-->
                            <!--            <td>{{x.name}}</td>-->
                            <!--            <td>{{x.ratio}}</td>-->
                            <!--            <td>-->
                                          
                            <!--                <a ng-click="priceEdit(x.id)" class="btn btn-info btn-xs">编辑</a>-->
                            <!--                <a class="btn btn-danger btn-xs" ng-click="delsPrice(x.id)">删除</a>-->
                            <!--            </td>-->
                            <!--        </tr>-->
                            <!--    </volist>-->

                            <!--    </tbody>-->
                            <!--</table>-->
                            
                            
                                </div>
                            </div>
                            <!--<div class="col-md-1">-->
                            <!--    <div class="bg-info form-block text-center" ng-click="changeBlank()">确认调整</div>-->
                            <!--</div>-->
                        </div>

                        <h5>板块设置</h5>
                        <!-- Nav tabs -->
                        <ul class="nav nav-tabs" role="tablist">
                            <li role="presentation" class="{{cat_index==1?'active':''}}" ng-click="changeType(1)"><a>板块一</a></li>
                            <li role="presentation" class="{{cat_index==2?'active':''}}" ng-click="changeType(2)"><a>板块二</a></li>
                            <li role="presentation" class="{{cat_index==3?'active':''}}" ng-click="changeType(3)"><a>板块三</a></li>
                        </ul>

                        <div class="row m-t-10" style="border-bottom: 1px solid #ddd; padding-bottom:10px;">
                            <!--<div class="col-md-2 text-center">-->
                            <!--    <br/><br/>-->
                            <!--    <button type="button" class="btn btn-primary btn-sm"  data-toggle="modal" data-target="#myBlank">设置板块图片</button>-->
                            <!--</div>-->
                            <div class="col-md-8">

                                <form class="form-inline m-b-10 row">
                                    <div class="form-group col-md-4 form-width">
                                        <label>板块名称</label>
                                        <input type="text" class="form-control input-sm" ng-model="blank.catname">
                                    </div>
                                    <div class="form-group">
                                         <label>执行税率</label>
                                        <select name="new_tax_id" ng-model="blank.new_tax_id" class="form-control">
                                            <option value="0">暂不选择</option>
                                             <option  value="{{v.id}}" ng-repeat="v in tax_list">{{v.name}}</option>
                                        </select>
                                    </div>
                                    <!--<div class="form-group col-md-4 form-width">-->
                                    <!--    <label>普票费率</label>-->
                                    <!--    <input type="text" class="form-control input-sm" ng-model="blank.ticket_nor">-->
                                    <!--</div>-->
                                    <!--<div class="form-group col-md-4 form-width">-->
                                    <!--    <label>专票费率</label>-->
                                    <!--    <input type="text" class="form-control input-sm" ng-model="blank.ticket_person">-->
                                    <!--</div>-->
                                    <div class="form-group col-md-4 form-width m-t-10">
                                        <button type="button" class="btn btn-default btn-xs" ng-click="changeBlank()">更新板块内容</button>
                                    </div>
                                </form>

                                <form class="form-inline m-b-10 row">
                                    <div class="form-group col-md-4 form-width">
                                        <label>商品图片</label>
                                        <input type="file" id="goods" name="goods" class="form-control input-sm">
                                    </div>
                                    <div class="form-group col-md-4 form-width">
                                        <label>banner</label>
                                        <input type="file" id="banner" name="banner" class="form-control input-sm">
                                    </div>
                                    <div class="form-group col-md-4 form-width">
                                        <label>商品名称</label>
                                        <input type="text" class="form-control input-sm" ng-model="blank.gnames">
                                    </div>

                                    <button type="button" class="btn btn-default btn-sm" ng-click="changeGoodsImgs()">更新板块商品</button>
                                    <a ng-click="blankClear()" class="btn btn-danger btn-sm m-l-10">清空板块数据</a>
                                </form>

                            </div>
                            <div class="col-md-2 text-center">
                                <p>banner</p>
                                <a href="javascript:;">
                                    <img src="/Public/uploads/banner/{{blank.source[0]}}" onclick="showimage(this)" style=" width:50%;"/>
                                </a>
                            </div>
                            
                            <div class="col-md-2 text-center">
                                <p>商品图片</p>
                                <a href="javascript:;">
                                    <img src="/Public/uploads/goods/{{blank.gimage}}"  onclick="showimage(this)" style=" width:50%;"/>
                                </a>
                            </div>

                        </div>
                        
   <div class="modal fade bs-example-modal-lg text-center" id="imgModal" tabindex="-1" role="dialog" data-toggle="modal" aria-labelledby="myLargeModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-lg" style="display:inline-block;width:auto;">
       <div class="modal-content">
           <div id="imgshow"></div>
        </div>
      </div>
    </div>

                        <h5 class="p-b-5">
                            商品设置
                            <a ng-click="goodsClear()" class="btn btn-danger btn-xs m-l-10 pull-right">清空商品数据</a>
                            <a ng-click="upGoodsAll()" class="btn btn-info btn-xs pull-right">更新商品</a>
                            <!--<a ng-click="addGoodsPre()" class="btn btn-info btn-xs pull-right m-r-10">添加商品</a>-->
                            <a class="btn btn-info btn-xs pull-right m-r-10" data-toggle="modal" data-target="#myImport">导入商品数据</a>
                            <!--<a class="btn btn-info btn-xs pull-right m-r-10" data-toggle="modal" data-target="#myTemp">添加商品模板数据</a>-->
                        </h5>

                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <td>#</td>
                                    <td><input type="text" class="form-control input-sm" id="keya" value="{{keys.keya}}"/></td>
                                    <td><input type="text" class="form-control input-sm" id="keyb" value="{{keys.keyb}}"/></td>
                                    <td><input type="text" class="form-control input-sm" id="keyc" value="{{keys.keyc}}"/></td>
                                    <td><input type="text" class="form-control input-sm" id="keyd" value="{{keys.keyd}}"/></td>
                                    <td><input type="text" class="form-control input-sm" id="keye" value="{{keys.keye}}"/></td>
                                    <td>综合价格</td>
                                    <td>溢价率</td>
                                    <td>执行价格</td>
                                    <td>发货时间</td>
                                    <!--<td width="30%">管理</td>-->
                                </tr>
                            </thead>
                            <tbody>

                                <tr ng-repeat=" x in list">
                                    <td>{{ $index + 1}}</td>
                                    <td><input type="text" class="form-control input-sm" ng-model="x.value_0"/></td>
                                    <td><input type="text" class="form-control input-sm" ng-model="x.value_1"/></td>
                                    <td><input type="text" class="form-control input-sm" ng-model="x.value_2"/></td>
                                    <td><input type="text" class="form-control input-sm" ng-model="x.value_3"/></td>
                                    <td><input type="text" class="form-control input-sm" ng-model="x.value_4"/></td>
                                    <td><input type="text" class="form-control input-sm" ng-model="x.price" disabled/></td>
                                    <td><input type="text" class="form-control input-sm" ng-model="x.dratio"/></td>
                                    <td><input type="text" class="form-control input-sm" value="{{x.market}}" disabled/></td>
                                    <td><input type="text" class="form-control input-sm" ng-model="x.trans"/></td>
                                    <!--<td>-->
                                    <!--    <a ng-click="changelogs(x.id, 1)" class="btn btn-info btn-xs">上调顺序</a>-->
                                    <!--    <a ng-click="changelogs(x.id, 0)" class="btn btn-info btn-xs">下调顺序</a>-->
                                    <!--    <a ng-click="dels(x.id)" class="btn btn-danger btn-xs">删除</a>-->
                                    <!--</td>-->
                                </tr>

                            </tbody>
                        </table>

                    </div>
                </div>
            </div>
        </div>
    </section>
    
        <!-- 价格组成Modal -->
    <div class="modal fade" id="myModalPrice" tabindex="-1" role="dialog" aria-labelledby="myModalPrice">
        <div class="modal-dialog" role="document" style="width:35%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">添加</h4>
                </div>
                <div class="modal-body">

                    <form name="ador">
                        <div class="form-group">
                            <label>名称</label>
                            <input type="text" name="name" ng-model="price.infos.name" class="form-control"/>
                        </div>

                        <div class="form-group">
                             <label>一级分类</label>
                            <select  name="price.infos.new_cate_one_id" ng-model="price.infos.new_cate_one_id" class="form-control">
                                <option value="0">暂不选择</option>
                                 <option  value="{{v.id}}" ng-repeat="v in price.new_cate_one_list">{{v.name}}</option>
                            </select>
                        </div>
                        <div class="form-group">
                             <label>二级分类</label>
                            <select name="price.infos.new_cate_two_id" ng-model="price.infos.new_cate_two_id" class="form-control">
                                <option value="0">暂不选择</option>
                                 <option  value="{{v.id}}" ng-repeat="v in price.new_cate_two_list">{{v.name}}</option>
                            </select>
                        </div>
                        <div class="form-group">
                             <label>详细材料</label>
                            <select name="pprice.infos.new_cate_three_id"  ng-model="price.infos.new_cate_three_id" class="form-control">
                                <option value="0">暂不选择</option>
                                 <option  value="{{v.id}}" ng-repeat="v in price.new_cate_three_list">{{v.name}}</option>
                            </select>
                        </div>
                        
                        <div class="form-group">
                            <label>溢价率</label>
                            <input type="number" name="ratio" ng-model="price.infos.ratio" class="form-control"/>
                        </div>
                    </form>

                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-primary" ng-click="priceAddPost()">添加</button>
                </div>
            </div>
        </div>
    </div>
    
        <!-- 价格组成Modaledit -->
    <div class="modal fade" id="myModalPriceEdit" tabindex="-1" role="dialog" aria-labelledby="myModalPriceEdit">
        <div class="modal-dialog" role="document" style="width:35%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">编辑</h4>
                </div>
                <div class="modal-body">

                    <form name="ador">
                        <div class="form-group">
                            <label>名称</label>
                            <input type="text" name="name" ng-model="price.infos.name" class="form-control"/>
                        </div>

                        <div class="form-group">
                             <label>一级分类</label>
                            <select  name="price.infos.new_cate_one_id" ng-model="price.infos.new_cate_one_id" class="form-control">
                                <option value="0">暂不选择</option>
                                 <option  value="{{v.id}}" ng-repeat="v in price.new_cate_one_list">{{v.name}}</option>
                            </select>
                        </div>
                        <div class="form-group">
                             <label>二级分类</label>
                            <select name="price.infos.new_cate_two_id" ng-model="price.infos.new_cate_two_id" class="form-control">
                                <option value="0">暂不选择</option>
                                 <option  value="{{v.id}}" ng-repeat="v in price.new_cate_two_list">{{v.name}}</option>
                            </select>
                        </div>
                        <div class="form-group">
                             <label>详细材料</label>
                            <select name="pprice.infos.new_cate_three_id"  ng-model="price.infos.new_cate_three_id" class="form-control">
                                <option value="0">暂不选择</option>
                                 <option  value="{{v.id}}" ng-repeat="v in price.new_cate_three_list">{{v.name}}</option>
                            </select>
                        </div>
                        
                        <div class="form-group">
                            <label>百分比率</label>
                            <input type="number" name="ratio" ng-model="price.infos.ratio" class="form-control"/>
                        </div>
                    </form>

                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-primary" ng-click="priceEditPost()">保存</button>
                </div>
            </div>
        </div>
    </div>

    <!-- 添加商品模板 -->
    <div class="modal fade" id="myTemp" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="width:50%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">添加商品模板数据</h4>
                </div>
                <div class="modal-body myTempBody">


                    <table class="table">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>名称</th>
                                <th>模板类型</th>
                                <th>状态</th>
                                <th width="15%">添加时间</th>
                                <th width="30%">管理</th>
                            </tr>
                        </thead>
                        <tbody>

                        <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><tr>
                                <th scope="row"><?php echo ($k); ?></th>
                                <td><?php echo ($v["name"]); ?></td>
                                <td><?php echo ($v['types']<2?'多商品模板':'单商品模板'); ?></td>
                                <td><?php echo ($v['status']>0?'正常':'暂停使用'); ?></td>
                                <td><?php echo (date('Y/m/d H:i:s',$v["times"])); ?></td>
                                <td>
                                    <a ng-click="addGoodsMol('<?php echo ($v["id"]); ?>')" class="btn btn-danger btn-xs">添加</a>
                                </td>
                            </tr><?php endforeach; endif; else: echo "" ;endif; ?>

                        </tbody>
                    </table>

                </div>
            </div>
        </div>
    </div>

    <!-- 导入商品-->
    <div class="modal fade" id="myImport" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="width:50%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">导入商品数据</h4>
                </div>
                <div class="modal-body">


                    <div class="form-group">
                        <label>上传文件</label>
                        <input type="file" name="file" id='file' class="form-control"/>
                    </div>
                    
                     <div class="form-group">
                        <!--<label>下载模板</label>-->
                        <a href="//app.elccc.cn/Public/file/导入商品模板.xlsx">点击下载模板</a>

                    </div>

                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-primary" ng-click="addGoodsTmp()">添加</button>
                </div>
            </div>
        </div>
    </div>

    <!-- 添加商品-多 -->
    <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="width:50%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">添加商品</h4>
                </div>
                <div class="modal-body">

                    <form class="row" name="goods">
                        <input type="hidden" name="pid" value="<?php echo ($id); ?>"/>
                        <input type="hidden" name="ptype" value="2"/>

                        <div class="col-md-6">
                            <div class="form-group">
                                <label>参数名称1</label>
                                <input type="text" name="key_0" class="form-control input-sm">
                            </div>
                            <div class="form-group">
                                <label>参数名称1内容</label>
                                <input type="text" name="value_0" class="form-control input-sm">
                            </div>
                            <div class="form-group">
                                <label>参数名称2</label>
                                <input type="text" name="key_1" class="form-control input-sm">
                            </div>
                            <div class="form-group">
                                <label>参数名称2内容</label>
                                <input type="text" name="value_1" class="form-control input-sm">
                            </div>
                            <div class="form-group">
                                <label>商品导体比例</label>
                                <input type="text" name="dratio" class="form-control input-sm">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>参数名称3</label>
                                <input type="text" name="key_2" class="form-control input-sm">
                            </div>
                            <div class="form-group">
                                <label>参数名称3内容</label>
                                <input type="text" name="value_2" class="form-control input-sm">
                            </div>
                            <div class="form-group">
                                <label>基础价格</label>
                                <input type="text" name="price" class="form-control input-sm">
                            </div>
                            <div class="form-group">
                                <label>发货时间</label>
                                <input type="text" name="trans" class="form-control input-sm">
                            </div>
                        </div>

                    </form>


                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-primary" ng-click="addGoods()">保存</button>
                </div>
            </div>
        </div>
    </div>

    <div class="modal fade" id="myBlank" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document" style="top:30%; width:50%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">设置商品板块轮播图</h4>
                </div>
                <div class="modal-body">

                    <form class="form-inline m-b-10 row">
                        <div class="form-group form-width col-md-4">
                            <label>板块图片</label>
                            <input type="file" id="filer" class="form-control input-sm">
                        </div>
                        <button type="button" class="btn btn-default btn-sm" ng-click="changeImgs()">添加板块图片</button>
                    </form>

                    <table class="table">
                        <caption>板块图片</caption>
                        <thead>
                            <tr class="text-center">
                                <th>#</th>
                                <th>图片</th>
                                <th>操作</th>
                            </tr>
                        </thead>
                        <tbody>

                            <tr  class="text-left" ng-repeat="x in blank.source">
                                <td>1</td>
                                <td><img src="/Public/uploads/banner/{{x}}" style=" width:50px;"/></td>
                                <td><a class="btn btn-xs btn-danger"  ng-click="deleteImgs($index)">删除</a></td>
                            </tr>

                        </tbody>
                    </table>

                </div>
            </div>
        </div>
    </div>

</div>

<script>

   function showimage(e)
     {
         var src = $(e).attr('src');
         
         $("#imgModal").find("#imgshow").html("<img src='"+src+"' class='carousel-inner img-responsive img-rounded' data-dismiss='modal'>");
         $("#imgModal").modal('show');

     }

    //轮播
    var swiper = new Swiper('.swiper-container', {slidesPerView: 3, spaceBetween: 10});
    var app = angular.module('myApp', []);
    app.controller('myCtrl', function ($scope, $http) {

        $scope.pid = "<?php echo ($id); ?>";
        $scope.cat_index = 1;
        $scope.blank = {
            catname: "", source: "",
            up_a: 0, down_a: 0, up_b: 0, down_b: 0, up_c: 0, down_c: 0,
            up_d: 0, down_d: 0, up_e: 0, down_e: 0, up_f: 0, down_f: 0
        };
        $scope.list = [];
        $scope.infos;
        $scope.keys;
        $scope.tax_list = [];
        $scope.price_list = [];
        ////////////////////////////////////////////////////////////////////////
        $scope.price = [];
        $scope.price.infos = [];
        $scope.price.infos.ratio = 1;
        $scope.price.new_cate_one_list = [];
        $scope.price.new_cate_two_list = [];
        $scope.price.new_cate_three_list = [];
        /**
         * 通讯操作
         * @param {type} url
         * @param {type} data
         * @returns {undefined}
         */
        $scope.commAjax = function (url, data, success) {
            $http({
                method: "POST",
                url: url,
                data: data,
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                transformRequest: function (obj) {
                    var str = [];
                    for (var p in obj) {
                        str.push(encodeURIComponent(p) + "=" + encodeURIComponent(obj[p]));
                    }
                    return str.join("&");
                }
            }).success(function (response) {
                success(response);
            });
        };
        /**
         * 通讯操作
         * @param {type} url
         * @param {type} data
         * @returns {undefined}
         */
        $scope.commAjax2 = function (url, data, success) {
            $http({
                method: "POST",
                url: url,
                data: data,
                headers: {'Content-Type': undefined},
                transformRequest: angular.identity,
            }).success(function (response) {
                success(response);
            });
        };
        
        $scope.priceAdd = function (id) {
             var cat_index = $scope.cat_index;
            $scope.commAjax("<?php echo U('goodsPrice/priceAddGet');?>", {plate_conts_id: $scope.pid,cat_index:cat_index}, function (res) {
               $scope.price.infos = res.data.info;
               $scope.priceCateList();
               $("#myModalPrice").modal("show");
            });
        }
        $scope.priceEdit = function (id) {
             var cat_index = $scope.cat_index;
            $scope.commAjax("<?php echo U('goodsPrice/priceEditGet');?>", {id:id}, function (res) {
               $scope.price.infos = res.data.info;
               $scope.priceCateList();
               $("#myModalPriceEdit").modal("show");
            });
        }
             //监听切换
        $scope.$watch('price.infos.new_cate_one_id', function(newVal,oldVal) {
            console.log(2233);
             console.log(oldVal);
            console.log(newVal);
    
          if(newVal != oldVal && newVal > 0){
              $scope.priceCateList(newVal)
          }
        
        });
        $scope.$watch('price.infos.new_cate_two_id', function(newVal,oldVal) {
            console.log(2233);
             console.log(oldVal);
            console.log(newVal);
    
          if(newVal != oldVal && newVal > 0){
              $scope.priceCateList(newVal)
          }
        
        });
       
        
        $scope.priceCateList = function (pid = 0) {
            var param = {pid: pid};
            //////
            $scope.commAjax("<?php echo U('goodsPrice/priceCateList');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                if (res.data.level != null) {
                    if(res.data.level == 1){
                        $scope.price.new_cate_one_list = res.data.cate_list;
                    }else if(res.data.level == 2){
                        $scope.price.new_cate_two_list = res.data.cate_list;
                    }else{
                        $scope.price.new_cate_three_list = res.data.cate_list;
                    }
                 

                } else {
                    
                }
            });
        };
        
        $scope.priceAddPost = function () {
            var param = $scope.price.infos;
            
            /////////
            $.ajax({
                url: "<?php echo U('goodsPrice/priceAddPost');?>",
                type: "post",
                dataType: 'json',
                data: param,
                success: function (data) {
                    if (data.status !== 1) {
                        return swal("错误", data.msg, "error");
                    }
                     $("#myModalPrice").modal("hide");
                     $scope.init();
                    // window.location.reload();
                },
                error: function (data) {
                    console.log(data);
                }
            });
        }
        
        $scope.priceEditPost = function () {
            var param = $scope.price.infos;
            
            /////////
            $.ajax({
                url: "<?php echo U('goodsPrice/priceEditPost');?>",
                type: "post",
                dataType: 'json',
                data: param,
                success: function (data) {
                    if (data.status !== 1) {
                        return swal("错误", data.msg, "error");
                    }
                     $("#myModalPriceEdit").modal("hide");
                    $scope.init();
                    // window.location.reload();
                },
                error: function (data) {
                    console.log(data);
                }
            });
        }
        
        $scope.delsPrice = function (ids) {
            var param = {id: ids};
            $scope.commAjax("<?php echo U('goodsPrice/delsPrice');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                $scope.init();
                // window.location.reload();
            });
        }
        
        
        /**
         * 调整比例
         * @returns {undefined}
         */
        $scope.init = function () {
            var param = {pid: $scope.pid, cat_index: $scope.cat_index};
            //////
            $scope.commAjax("<?php echo U('Goods/plate_conts_blankr');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                if (res.data.info != null) {
                    $scope.blank = res.data.info;
                    $scope.list = res.data.list;
                    $scope.keys = res.data.key;
                    $scope.tax_list = res.data.tax_list;
                    $scope.price_list = res.data.price_list;
                } else {
                    $scope.blank = {
                        catname: "", source: "",
                        up_a: 0, down_a: 0, up_b: 0, down_b: 0, up_c: 0, down_c: 0,
                        up_d: 0, down_d: 0, up_e: 0, down_e: 0, up_f: 0, down_f: 0
                    };
                    $scope.list = [];
                }
            });
        };
        $scope.init();
        ////////////////////////////////////////////////////////////////////////

        /**
         * 更换类型
         * @param {type} index
         * @returns {undefined}
         */
        $scope.changeType = function (index) {
            $scope.cat_index = index;
            $scope.init();
        };
        /**
         * 调整板块图片
         * @returns {undefined}
         */
        $scope.changeImgs = function () {
            var param = new FormData();
            param.append('file', document.getElementById("filer").files[0]);
            param.append('pid', $scope.pid);
            param.append('cat_index', $scope.cat_index);
            //////
            $scope.commAjax2("<?php echo U('Goods/plate_conts_upload');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                $scope.init();
                return swal("提示", res.msg, "success");
            });
        };

        /**
         * 删除板块图片
         * @returns {undefined}
         */
        $scope.deleteImgs = function (i) {
            var param = {pid: $scope.pid, cat_index: $scope.cat_index, index: i};
            $scope.commAjax("<?php echo U('Goods/plate_conts_del');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                $scope.init();
            });
        }

        /**
         * 调整板块比例及基本信息
         * @returns {undefined}
         */
        $scope.changeBlank = function () {
            var param = $scope.blank;
            param.pid = $scope.pid;
            param.cat_index = $scope.cat_index;
            //////
            $scope.commAjax("<?php echo U('Goods/plate_conts_blank');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                $scope.init();
                return swal("提示", res.msg, "success");
            });
        };
        /**
         * 调整板块商品图片
         * @returns {undefined}
         */
        $scope.changeGoodsImgs = function () {
            var param = new FormData();
            param.append('file', document.getElementById("goods").files[0]);
            param.append('banner', document.getElementById("banner").files[0]);
            param.append('pid', $scope.pid);
            param.append('cat_index', $scope.cat_index);
            angular.forEach($scope.blank, function (key, value) {
                param.append(value, key);
            });
            //////
            $scope.commAjax2("<?php echo U('Goods/plate_conts_gimgs');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                $scope.init();
                return swal("提示", res.msg, "success");
            });
        };
        ////////////////////////////////////////////////////////////////////////

        /**
         * 添加模板参数
         * @returns {undefined}
         */
        $scope.addGoodsMol = function (i) {
            var param = {pid: $scope.pid, cat_index: $scope.cat_index, ptype: 2, ids: i};
            ///
            layer.load(2);
            $scope.commAjax("<?php echo U('Goods/plate_conts_list_temp');?>", param, function (res) {
                layer.closeAll();
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                $("#myTemp").modal("hide");
                $scope.init();
            });
        }

        /**
         * 导入商品数据
         * @returns {undefined}
         */
        $scope.addGoodsTmp = function () {
            var param = {pid: "<?php echo ($id); ?>", cat_index: $scope.cat_index, ptype: 2};
            layer.load(2);
            /////////
            $.ajaxFileUpload({
                url: "<?php echo U('Goods/plate_conts_import');?>",
                secureuri: false,
                fileElementId: "file",
                dataType: 'json',
                data: param,
                success: function (data) {
                    layer.closeAll();
                    if (data.status !== 1) {
                        return swal("错误", data.msg, "error");
                    }
                    $("#myImport").modal("hide");
                    $scope.init();
                },
                error: function (data) {
                    console.log(data);
                }
            });
        }

        $scope.addGoodsPre = function () {
            var param = {pid: "<?php echo ($id); ?>", cat_index: $scope.cat_index, ptype: 2};
            layer.load(2);
            $scope.commAjax("<?php echo U('Goods/plate_conts_listCov');?>", param, function (res) {
                layer.closeAll('loading');
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                if (res.data > 0) {
                    return $scope.init();
                }
                return $("#myModal").modal("show");
            });
        }

        /**
         * 添加商品
         * @returns {undefined}
         */
        $scope.addGoods = function () {
            var temp = $("form[name='goods']").serializeArray();
            var data = objToArray(temp);
            data.market = data.price;
            data.cat_index = $scope.cat_index;
            /////////
            layer.load(2);
            $.ajaxFileUpload({
                url: "<?php echo U('Core/addon','model=plate_conts_logs');?>",
                secureuri: false,
                fileElementId: "filer",
                dataType: 'json',
                data: data,
                success: function (data) {
                    layer.closeAll('loading');
                    if (data.status !== 1) {
                        return swal("错误", data.msg, "error");
                    }
                    window.location.reload();
                },
                error: function (data) {
                    console.log(data);
                }
            });
        };
        /**
         * 批量更新
         * @returns {undefined}
         */
        $scope.upGoodsAll = function () {
            layer.load(2);
            var param = {
                pid: "<?php echo ($id); ?>",
                cat_index: $scope.cat_index,
                logs: JSON.stringify($scope.list),
                keya: $("#keya").val(),
                keyb: $("#keyb").val(),
                keyc: $("#keyc").val(),
                keyd: $("#keyd").val(),
                keye: $("#keye").val(),
            };
            $scope.commAjax("<?php echo U('Goods/plate_conts_update');?>", param, function (res) {
                layer.closeAll('loading');
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                return $scope.init();
                //return swal("提示", "更新成功", "success");
            });
        }

        $scope.dels = function (ids) {
            var param = {id: ids};
            layer.load(2);
            $scope.commAjax("<?php echo U('Core/dels','model=plate_conts_logs');?>", param, function (res) {
                layer.closeAll('loading');
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                return $scope.init();
            });
        }

        ////////////////////////////////////////////////////////////////////////

        /**
         * 清空板块
         * @returns {undefined}
         */
        $scope.blankClear = function () {
            var param = {pid: "<?php echo ($id); ?>", cat_index: $scope.cat_index, ptype: 2};
            layer.load(2);
            $scope.commAjax("<?php echo U('Goods/plate_conts_clearblank');?>", param, function (res) {
                layer.closeAll('loading');
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                return $scope.init();
            });
        }
        
        /**
         * 清空商品
         * @returns {undefined}
         */
        $scope.goodsClear = function () {
            var param = {pid: "<?php echo ($id); ?>", cat_index: $scope.cat_index, ptype: 2};
            layer.load(2);
            $scope.commAjax("<?php echo U('Goods/plate_conts_clearlog');?>", param, function (res) {
                layer.closeAll('loading');
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                return $scope.init();
            });
        }

        $scope.changelogs = function (id, type) {
            var param = {id: id, redi: type};
            $scope.commAjax("<?php echo U('Core/uplogs');?>", param, function (res) {
                if (res.status !== 1) {
                    return swal("错误", res.msg, "error");
                }
                $scope.init();
            });
        }
        
        $scope.copyPrice = function (ids) {
            swal({
                title: '确认',
                text: "确认复制上一版块的内容吗？",
                icon: 'warning',
                // buttons: true,
                showCancelButton: true, // 显示取消按钮
                confirmButtonText: '确认', // 确认按钮文本
                cancelButtonText: '取消', // 取消按钮文本
                // dangerMode: true,
           }).then((willDelete) => {
                if (willDelete) {
                      var param = {pid: "<?php echo ($id); ?>", cat_index: $scope.cat_index, ptype: 2};
                    // var param = {id: ids};
                    $scope.commAjax("<?php echo U('GoodsPrice/copyPrice');?>", param, function (res) {
                        if (res.status !== 1) {
                            return swal("错误", res.msg, "error");
                        }
                        $scope.init();
                        // window.location.reload();
                    });
                }else{
                    
                }
            
            })   
        }
        
        $scope.delAllPrice = function (ids) {
            swal({
                title: '确认',
                text: "确认删除吗？",
                icon: 'warning',
                // buttons: true,
                showCancelButton: true, // 显示取消按钮
                confirmButtonText: '确认', // 确认按钮文本
                cancelButtonText: '取消', // 取消按钮文本
                // dangerMode: true,
           }).then((willDelete) => {console.log(1122111);console.log(willDelete);
                if (willDelete) {
                      var param = {pid: "<?php echo ($id); ?>", cat_index: $scope.cat_index, ptype: 2};
                    // var param = {id: ids};
                    $scope.commAjax("<?php echo U('GoodsPrice/delAllPrice');?>", param, function (res) {
                        if (res.status !== 1) {
                            return swal("错误", res.msg, "error");
                        }
                        $scope.init();
                        // window.location.reload();
                    });
                }else{
                    
                }
            
            })   
        }

    });


</script>

            </div>
        </div>
        <!-- nano scroller -->
        <script src="/Public/assets/js/lib/menubar/sidebar.js"></script>
        <script src="/Public/assets/js/lib/preloader/pace.min.js"></script>
        <!-- scripit init-->
        <script type="text/javascript" src="/Public/assets/js/scripts.js"></script>
    </body>

</html>