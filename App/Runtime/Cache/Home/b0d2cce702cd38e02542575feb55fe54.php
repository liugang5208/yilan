                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td>
                                            <span style="display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 12px; font-weight: 500; background-color: <?php echo ($v["status gt 0 ? 'rgba(52, 199, 89, 0"]["1)' : 'rgba(255, 59, 48, 0"]["1)'"]); ?>; color: <?php echo ($v["status gt 0 ? '#34c759' : '#ff3b30'"]); ?>;">
                                                <?php echo ($v["status gt 0 ? '正常' : '暂停使用'"]); ?>
                                            </span>
                                        </td>
                                        <td>
                        ...
                        <div class="mobile-card-list">
                            <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="m-card-item">
                                    <div class="m-card-header">
                                        <span class="m-card-title"><?php echo ($v["title"]); ?></span>
                                    ...
                                    <div class="m-card-footer">
                                        <a class="btn btn-info" ng-click="updateinfo('<?php echo ($v["id"]); ?>')">编辑</a>
                                    </div>
...
<script>
    ...
    function addor() {
        ...
        var baseurl = "<?php echo U('Core/addon','model=bar');?>";
        $.ajax({
            url: baseurl,
            type: "post",
            data: data,
    ...
    app.controller('myCtrl', function ($scope, $http) {
        ...
        $scope.updateinfo = function (id) {
            $scope.commAjax("<?php echo U('Core/infos','model=bar');?>", {ids: id}, function (res) {
        ...
        $scope.updateCats = function () {
            ...
            param.ids = param.id;
            $.ajax({
                url: "<?php echo U('Core/edits','model=bar');?>",
                type: "post",
                dataType: 'json',
                data: param,
        ...
        $scope.dels = function (ids) {
            var param = {id: ids};
            $scope.commAjax("<?php echo U('Core/dels','model=bar');?>", param, function (res) {
                if (res.status !== 1) {
